...
@router.get("/product-images/{image_id}", tags=["product-images"], description="Get a product image by id", response_model=DataResponse[ProductImageSchema])
async def get_product_image(image_id: int, db: Session = Depends(get_db)):
    product_image = db.query(ProductImage).filter(ProductImage.image_id == image_id).first()
...
@router.put("/product-images/{image_id}", tags=["product-images"], description="Update a product image by id", response_model=DataResponse[ProductImageSchema])
async def update_product_image(image_id: int, data: CreateProductImageSchema, db: Session = Depends(get_db)):
    product_image = db.query(ProductImage).filter(ProductImage.image_id == image_id).first()
    if not product_image:
        return DataResponse.custom_response(
            code="404", message="Product image not found", data=None
...
@router.delete("/product-images/{image_id}", tags=["product-images"], description="Delete a product image by id", response_model=DataResponse[None])
